...
# その他:[データ API は最も近いリードレプリカにルートします](https://supabase.com/blog/data-api-nearest-read-replica)

本日、API ロードバランサーを拡張して地理ルーティングを処理できるようにすることで、最も近いリードレプリカにルーティングする Data API リクエストをリリースします。
これは、グローバルに分散されたアプリケーションのリクエストレイテンシを最小限に抑える、非常に効果的な改善です。ロードバランサーエンドポイントを使用する場合は、デフォルトで利用できます。
### ジオルーティングとは
ジオルーティングにより、Data API リクエストが地理的に最も近いデータベースの読み取りレプリカに自動的に送信されるため、レイテンシが短縮され、世界中のユーザーに対する応答時間が向上します。
![](/images/supabase-loanch-week/image15.png)
以前は、フランクフルト、シンガポール、バージニアに読み取りレプリカがあった場合、ヨーロッパのユーザーはいずれかのレプリカにリクエストを送信する可能性があるため、レイテンシが大幅に異なる可能性がありました。
新しいジオルーティングにより、ユーザーは最も近いリードレプリカに自動的に接続されるため、同じユーザーはフランクフルト地域のリードレプリカにのみリクエストを送信します。
### ジオルーティングの仕組み
当社の地理ルーティング システムは、地理空間アルゴリズムを使用して、各リクエストに最適な読み取りレプリカを決定します。
1. 受信する各 API リクエストには、ネットワーク エッジからの地理位置情報データ (具体的にはcf.colo、リクエストを受信したデータセンターの IATA 空港コードを提供するプロパティ) が含まれます。
1. 当社では、読み取りレプリカを展開できる各リージョンを正確な地理空間座標に関連付ける座標マッピング システムを維持しています。
1. リクエストが到着すると、Haversine 式(経度と緯度を使用して球面上の 2 点間の大圏距離を決定する) を使用して、ネットワーク エッジと利用可能な各読み取りレプリカ間の距離を計算します。
1. システムはリクエストを地理的に最も近い読み取りレプリカに自動的にルーティングし、ユーザー側で構成を行うことなくネットワークの遅延を最小限に抑えます。
1. 同じリージョンに複数のデータベースが存在する場合は、バランスの取れた負荷分散を確保するためにラウンドロビン戦略を実装します。
プロセス全体はアプリケーションとユーザーにとって完全にシームレスであり、<project_ref>-all.supabase.co今日プロジェクト URL ( ) を更新する以外にコードや構成を変更する必要はありません。

ジオルーティングを最大限に活用するには、ユーザーが集中しているリージョンにリードレプリカを配置してください。リードレプリカを戦略的に配置するほど、レイテンシの短縮と応答時間の改善によるメリットがユーザーにもたらされます。
### 初期リリースとロードマップ
初期リリースでは、ジオルーティングは次の制限付きで利用できます。
:::message
現在、読み取り専用のデータ API (PostgREST) リクエストに制限されています
:::
現在、読み取り専用のデータ API (PostgREST) リクエストに制限されています
すでに API ロード バランサーを使用している場合は、何もする必要はありません。地理ルーティングは Data API リクエストに自動的に適用されます。
それ以外の場合は、プロジェクトが API ロードバランサエンドポイント ( <project_ref>-all.supabase.co)を使用していることを確認することで、この機能を有効にすることができます。

# その他:[専用プーラー](https://supabase.com/blog/dedicated-poolers)
本日、私たちは専用プーラーを発表します。これは、パフォーマンスと信頼性を最大限に高めるためにデータベースと同じ場所に配置される Postgres 接続[プーラー](https://supabase.com/docs/guides/database/connecting-to-postgres#serverside-poolers)です。
### Supabase専用プーラー
専用プーラーは、 Postgresデータベースと同じ場所に設置されるPgBouncerインスタンスです。IPv6で接続する必要がありますが、それが不可能な場合はIPv4アドオンをご利用ください。
専用プーラーは独自のプロジェクトに分離されており、構成を細かく制御できます。
### データベースへの接続
これにより、データベースに接続するための3つのオプションが提供されます。
![](/images/supabase-loanch-week/image16.png)
- 直接接続（無料）：サーバーを使用して接続する場合に推奨されます。
- Shared Pooler（無料）：サーバーレス関数（Next.js や AWS Lambda など）を使用して接続する場合に推奨されます。
- 専用プーラー（Proプラン以上）：サーバーレス関数を使用して接続し、スケールアップを開始する場合におすすめです。Proプラン以上でご利用いただけます。
### SupavisorとPgBouncerの選択
1年前、AWSはIPv4アドレスの課金を開始しました。ネットワークがIPv6をサポートしていない場合でも、データベースへの接続に料金が発生しないようにするために、 Supavisorを導入しました。世界でIPv6を採用しているのは約45%に過ぎないため、これは多くの人にとって素晴らしいソリューションでした。
![](/images/supabase-loanch-week/image17.png)
ここ数ヶ月、私たちのプラットフォームは前例のない成長を遂げています。毎週何万人もの新しい開発者がSupabaseに参入し、データベースを使って奇妙で素晴らしいことを行っています。
![](/images/supabase-loanch-week/image18.png)
専用プーラーの導入により、ユースケースに最適な接続タイプを柔軟に選択できるようになります。専用ハードウェアが必要な場合は、Proプラン以上で専用プーラーをご利用いただくことで、低レイテンシ、優れたパフォーマンス、そして高い信頼性を実現できます。