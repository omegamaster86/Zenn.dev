...
# ここが良いぞ仕様書駆動開発
### 実装、要件漏れに気づきやすい
経験が浅い方からベテラン問わず、誰でも忘れてしまうことはあると思います。そんな中でもドキュメント（仕様書）に基づいて開発を行えば実装、要件漏れに気づいて手戻りなく開発できます。
また業務理解が出来ているかの確認にもなります。お客様と話していないエンジニアにとってお客様の業務は想像でしかわかりませんが、仕様書という形で業務や作業を言葉にして残すことで深い業務理解がかのうになります。

### ドキュメント（仕様書）が古くなるのを防ぐ
よくある話だと思いますが、仕様書はNotion or Googleで管理していると実装中に追加要件が発生した場合、実装は最新の要件が反映され、仕様書は古いままになっている方や会社も多いのではありませんか？そんな中でも実装と仕様書が近くにあれば更新も忘れずに出来ますよね（意識はしないとだめですが）

### 経験が浅いエンジニアの味方
経験が浅いエンジニアの方にとってテーブル定義？何書くの？ER図？なにそれ美味しいの？
って感じだと思います。これは仕様書駆動開発というよりcursor-ssd-packageを使用すると、「これがテーブル定義か、こういうカラムが必要なんだな」とわかることができます。これって地味に頼もしくないですか？
...
::::
ではタスクを実行していきましょう`/impl 1.1`と入力するとタスクの1.1を実行してくれます。ちょっとずつ実行していく方が、レビューもしやすく、確認する側も気が重くならないので、良いですよね〜まぁあとはタスクを消化していくだけなので、以降は省略させていただきます。
気になった方は是非インストールをして遊んでみてください。
